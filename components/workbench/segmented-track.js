(()=>{
  ...
  class TimelineControls extends HTMLElement{
    ...
    _applyTheme(theme,persist){const resolved=theme==='light'?'light':'dark',preview=document.body.classList.contains('component-preview'),themeKey=preview?'workbench-component-theme':'workbench-theme';document.documentElement.classList.toggle('theme-light',resolved==='light');document.documentElement.dataset.theme=resolved;document.body.classList.toggle('theme-light',resolved==='light');document.body.classList.toggle('component-dark',resolved==='dark'&&preview);document.body.classList.remove('theme-pending');this.querySelectorAll('[data-theme]').forEach(button=>button.classList.toggle('is-active',button.dataset.theme===resolved));if(persist)try{localStorage.setItem(themeKey,resolved);}catch(_){}}
  }
  ...
  class WorkbenchMediaViewer extends HTMLElement{
    ...
    _render(){
      ...
      const resize=()=>{
        ...
        if(threePanel){
          const armWidth=Math.max(0,Math.floor((width-gap*2.5)/3)),armHeight=Math.floor(armWidth/1.5),layoutHeight=armHeight*2+gap,largeWidth=Math.floor(layoutHeight*1.5);
  ...
  class WorkbenchSegmentListPanel extends HTMLElement{
    ...
    setVariant(value,emit=true){
      ...
      this.querySelector('[data-review-title]').textContent={segments:'标注列表',log:'日志',info:'基本信息'}[variant];
