...
class TaskDataTable extends HTMLElement {
  connectedCallback(){
    ...
    this.addEventListener("click",event=>{const action=event.target.closest("[data-action]");const page=event.target.closest("[data-page]");if(action){this.dispatchEvent(new CustomEvent("row-action",{bubbles:true,detail:{action:action.dataset.action,row:Number(action.closest("tr")?.dataset.row)}}));if(action.dataset.action==="edit"&&this.getAttribute("editor-url")){const url=new URL(this.getAttribute("editor-url"),location.href);if(this.getAttribute("variant")==="process-management"&&action.closest("tr")?.dataset.row==="1")url.pathname=url.pathname.replace(/index\.html$/, "simple.html");location.href=url.href;}}if(page)this.selectPage(page.dataset.page);});
  }
  render(){
    if(this.getAttribute("variant")==="process-management"){
      const data=[
        ...
        {id:"e2e-split-annotation",name:"端到端切分标注简洁版",status:"draft",statusText:"草稿",time:"2026-08-04 11:30"},
        {id:"e2e-split-annotation-complex",name:"多阶段任务处理复杂版",status:"draft",statusText:"草稿",time:"2026-09-07 10:16"}
      ];
